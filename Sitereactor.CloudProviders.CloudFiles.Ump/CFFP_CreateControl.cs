...
{
    public class CFFP_CreateControl : AbstractCreateControl
    {
        private readonly CFFP_Provider _provider;
        ...
        public CFFP_CreateControl(CFFP_Provider provider, string options)
        {
            _provider = provider;
            _config = options.DeserializeJsonTo<CFFP_Config>();
        }
        ...
        public override bool TrySave(out MediaItem savedMediaItem, out string message)
        {
            ...
            if (_fileFileUpload.HasFile)
            {
                try
                {
                    savedMediaItem = _provider.CreateMediaItem(_fileFileUpload.FileContent, _fileFileUpload.FileName,
                        _titleTextBox.Text,
